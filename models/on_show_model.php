	...
	class On_Show_Model{
		...
		public function getSuburb($suburb_id){
			try{
				...
				$suburb = new Suburb();
				//$suburb->setMunicipalityID($row['municipality_id']);
		...
		public function getSuburb1($suburb_id){
			try{
		...
		*/
		...
		//Get suburb on show by property ID (Light Gallery)
		public function getPropertySuburbForSale($property_id){
			$query = "SELECT s.*, p.property_id, p.price, p.property_desc, c.city_id, c.city_name
					  FROM suburbs s
		...
		//Get suburb to rent by property ID (Light Gallery)
