<?php
	class Ndlambe extends Controller{
		...
		public function on_show_suburb($suburb_id){	
			...
			$this->view->render('ndlambe/on_show_suburb/index');	
		}
	
		public function to_rent_suburb($suburb_id){	
			$suburb = $this->model->getSuburb($suburb_id);
			$this->view->mndlambe = $suburb;
			$this->view->getSuburbImage = $this->model->getSuburbImage($suburb_id);
			if (empty($this->view->ndlambe)) {
					die('This is an invalid suburb!');
			}
			$this->view->render('ndlambe/to_rent_suburb/index');	
		}
		
		//Get single city
		public function city($city_id){
			$this->view->ndlambe = $this->model->getCity($city_id);
			$this->view->getSuburbs = $this->model->getSuburbs();
			if (empty($this->view->ndlambe)) {
			die('This is an invalid city!');
			}
											
			$this->view->render('ndlambe/city');
		}
		
		public function on_show_city($city_id){	
			$city = $this->model->getCity($city_id);
			$this->view->ndlambe = $city;
			$this->view->getSuburbImage = $this->model->getSuburbImage($city_id);
			if (empty($this->view->ndlambe)) {
				die('This is an invalid city!');
			}
			$this->view->render('ndlambe/on_show_city/index');	
		}
		...
		public function to_rent_city($city_id){	
			...
			$this->view->render('ndlambe/to_rent_city/index');	
		}
		
		//Get single image
		public function gallery($property_id){
			$this->view->ndlambe = $this->model->getAgentPropertyImagesForSale($property_id);
			...
			$this->view->render('ndlambe/gallery');
		}
		
		public function on_show_gallery($property_id){
			$this->view->ndlambe = $this->model->getAgentPropertyImagesOnShow($property_id);
			...
			$this->view->render('ndlambe/on_show_gallery/index');
		}
		
		public function to_rent_gallery($property_id){
			$this->view->ndlambe = $this->model->getAgentPropertyImagesToRent($property_id);
			...
			$this->view->render('ndlambe/to_rent_gallery/index');
		}		
	}
