<?php
	class PaymentDB{
		...
		//Get agent's payment
		public function getPayment($agent_id){
			...
			try{
					...
					$saleListing->setSaleListingID($row['sale_listing_id']);
					$saleListing->setSaleListingAmount($row['sale_listing_amount']);
					$saleListing->setTotalListing($row['total_listing']);
					
					/*
					$payment = new Payment();
					$payment->setAgent($agent);
					$payment->setProperty($property);
					$payment->setSaleListing($saleListing);
					$payment->setRentalListing($rentalListing);
					*/
					return $saleListing;
