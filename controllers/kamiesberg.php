<?php
	class Kamiesberg extends Controller{
		...
		public function suburb($suburb_id){
			...
			$this->view->kamiesberg = $suburb;
			$this->view->getSuburbImage = $this->model->getSuburbImage($suburb_id);
			if (empty($this->view->kamiesberg)) {
				die('This is an invalid suburb!');
			}
										
			$this->view->render('kamiesberg/suburb');
		}
		...
		public function on_show_suburb($suburb_id){	
			...
			$this->view->render('kamiesberg/on_show_suburb/index');	
		}
		...
		public function to_rent_city($city_id){	
			...
			$this->view->render('kamiesberg/to_rent_city/index');	
		}
		
		//Get single image
		public function gallery($property_id){
			$this->view->kamiesberg = $this->model->getAgentPropertyImagesForSale($property_id);
			...
			$this->view->render('kamiesberg/gallery');
		}
		
		public function on_show_gallery($property_id){
		$this->view->kamiesberg = $this->model->getAgentPropertyImagesOnShow($property_id);
		...
		$this->view->getPropertyAgent = $this->model->getPropertyAgentOnShow($property_id);
			...
			$this->view->render('kamiesberg/on_show_gallery/index');
		}
		
		public function to_rent_gallery($property_id){
		$this->view->kamiesberg = $this->model->getAgentPropertyImagesToRent($property_id);
		...
		$this->view->getPropertyAgent = $this->model->getPropertyAgentToRent($property_id);
			...
			$this->view->render('kamiesberg/to_rent_gallery/index');
		}
	}