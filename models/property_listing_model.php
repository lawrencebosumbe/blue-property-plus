	...
	//Add Property 
		public function addProperty(){
			try{
			  ...
			  if((!empty($agent_id)) || (!empty($suburb)) || (!empty($city)) || (!empty($municipality)) || 
			     (!empty($property_type)) || (!empty($street_no)) || (!empty($street_name)) ||(!empty($desc)) || 
				 (!empty($property_status)) || (!empty($num_bathrooms)) || (!empty($num_beds)) || (!empty($num_garages)) || 
				 (!empty($num_lounges)) || (!empty($air_con)) || (!empty($pool)) || (!empty($cottage)) || (!empty($price))){
				 if((isset($_POST["property_type"] ) ? $_POST["property_type"]: '') ||
				  ...
				  $query = "INSERT INTO properties (agent_id, suburb_id, city_id, municipality_id, property_type, 
				            property_status, street_no, street_name, property_desc, num_bathrooms, num_beds, num_garages, 
						    num_lounges, air_con, pool, cottage, price)
							VALUES('$agent_id', '$suburb', '$city', '$property_type', '$property_status', '$street_no', 
								   '$street_name', '$desc', '$num_bathrooms', '$num_beds', '$num_garages', 
								   '$num_lounges', '$air_con', '$pool', '$cottage', '$price')";
				  ...
				  if($row_count){
						...
						$sql = "UPDATE cities SET total_properties = total_properties + 1
								  WHERE city_id = '$city'";
						$row_count = $this->conn->exec($sql);
						
						$sql_query = "UPDATE municipalities SET total_properties = total_properties + 1
